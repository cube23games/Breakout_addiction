...
if not failures:
    with tempfile.TemporaryDirectory(
        ...
        if not icon.is_file():
            failures.append(
                'solid notification vector was not generated'
            )
        else:
            try:
                ...
                paths = list(vector_root.findall('path'))

                if len(paths) != 1:
                    failures.append(
                        'notification vector must contain exactly '
                        'one bold filled path'
                    )
                else:
                    path = paths[0]
                    ...
                    if fill != '#FFFFFFFF':
                        failures.append(
                            'notification path must be solid white'
                        )

                    if not data.strip():
                        failures.append(
                            'notification pathData is empty'
                        )
...
if failures:
    print('BA-56D verification failed:')
...
print(
    'BA-56D verification passed: Rescue completion prefers an exact '
    'alarm, retains an honest inexact fallback, verifies its pending '
    'request, cancels late foreground fallbacks, generates one bold '
    'solid notification glyph, protects it from resource shrinking, '
    'and makes CI inspect the release APK.'
)
