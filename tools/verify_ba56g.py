    ...
    with tempfile.TemporaryDirectory(
        ...
        if not icon.is_file():
            ...
            try:
                ...
                if len(vector_paths) != 5:
                    failures.append(
                        'notification vector must contain exactly five '
...
if failures:
    print('BA-56G verification failed:')
...
print(
    'BA-56G verification passed: Rescue completion prefers an exact '
    'alarm, retains an honest inexact fallback, verifies its pending '
    'request, cancels late foreground fallbacks, restores a '
    'recognition-focused brand-faithful notification silhouette, '
    'protects it from '
