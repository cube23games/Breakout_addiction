...
class MyReasonsRepository {
  ...
  Future<MyReasonsState> getState() async {
    ...
    if (decoded.isNotEmpty) {
      return MyReasonsState.fromMap(decoded);
    }
  ...
  Future<void> saveState(MyReasonsState state) async {
    ...
    await prefs.setString(
      _storageKey,
      jsonEncode(
        state.copyWith(updatedAt: DateTime.now().toUtc()).toMap(),
      ),
    );
  }

  Future<ReasonMediaItem> importPhoto(XFile picked) async {
    final directory = await getApplicationDocumentsDirectory();
    final mediaDirectory = Directory('${directory.path}/breakout_reasons');
    await mediaDirectory.create(recursive: true);
    final extension = _safeExtension(picked.path, fallback: '.jpg');
    final id = DateTime.now().microsecondsSinceEpoch.toString();
    final destination = File('${mediaDirectory.path}/reason_$id$extension');
    await File(picked.path).copy(destination.path);
    return ReasonMediaItem(
      id: id,
      path: destination.path,
      type: ReasonMediaType.photo,
    );
  }
